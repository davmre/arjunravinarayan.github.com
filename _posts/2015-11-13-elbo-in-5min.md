...
A natural approach to fitting the approximation parameters $\lambda$ is to minimize the KL divergence between our approximation $q(z;\lambda)$ and the posterior `$p(z|x)$`.[^2] If we just write down that divergence and unpack it using some simple algebra,
...
we see that it is equal to the model evidence $\log p(x)$, which is an (unknown) constant, minus a term $\mathcal{F}$ given by
